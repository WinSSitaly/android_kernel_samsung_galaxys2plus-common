...
static struct wake_lock alarm_rtc_wake_lock_irq;
...
#if defined(CONFIG_ARCH_CAPRI) //In case of Broadcom Project.
...
static void alarm_triggered_func(void *p)
{
	...
	if (AUDCTRL_InVoiceCall()) 
		wake_lock_timeout(&alarm_rtc_wake_lock_irq, 1 * HZ);
	else
	#endif
		wake_lock_timeout(&alarm_rtc_wake_lock, 1 * HZ);
}
...
static int __init alarm_driver_init(void)
{
	...
	wake_lock_init(&alarm_rtc_wake_lock_irq, WAKE_LOCK_SUSPEND, "alarm_rtc_irq");
...
err2:
	...
	wake_lock_destroy(&alarm_rtc_wake_lock_irq);
